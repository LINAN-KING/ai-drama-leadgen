...
export async function renderEdl(
  ...
  const clipsDirectory = path.join(options.workDirectory, "normalized-clips");
  const clips: string[] = [];
  ...
  const concatPath = path.join(options.workDirectory, "video-concat.txt");
  ...
  const silentVideo = path.join(options.workDirectory, "silent-video.mp4");
  await runBinary(
    "ffmpeg",
  ...
  const subtitleFilter = options.subtitlePath
    ? `subtitles='${options.subtitlePath.replaceAll("\\", "/").replaceAll(":", "\\:").replaceAll("'", "\\'")}'`
    : null;
  args.push(
  ...
  if (options.audioPath) args.push("-c:a", "aac", "-b:a", "192k", "-shortest");
  else args.push("-an");
